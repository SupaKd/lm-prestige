...
const avantages = [
  ...
  },
  {
    ...
    texte: 'Tous nos véhicules sont couverts tous risques. Vous conduisez l\'esprit tranquille.',
  },
  {
    icone: <Clock size={22} />,
    titre: 'Disponible 7j/7',
    texte: 'Service de location disponible tous les jours, y compris week-end et jours fériés.',
  },
  {
    icone: <MapPin size={22} />,
    titre: 'Zone Gex & Genève',
    texte: "Livraison possible à Gex, Ferney-Voltaire, Divonne et à l'aéroport de Genève.",
  },
]
  ...
  useEffect(() => {
    const onScroll = () => {
      ...
      rafRef.current = requestAnimationFrame(() => {
        ...
        if (photoRef.current)
          photoRef.current.style.transform = `scale(1.08) translateY(${p * 80}px)`

        // Phase 2 (0.5→1) : rideau noir descend
        const pRideau = Math.max(0, (p - 0.5) / 0.5)
        if (ridеauRef.current)
          ridеauRef.current.style.transform = `translateY(${(1 - pRideau) * -100}%)`
      })
    }
    ...
    <main className="page">
        ...
        <div className="hero__body" ref={bodyRef}>
          ...
          <h1 className="hero__titre">
            <span className="hero__titre-ligne" ref={ligne1Ref}>
              <span className="hero__mot" style={{ '--d': '0s' }}>Louez</span>
              <span className="hero__mot" style={{ '--d': '0.08s' }}>le</span>
              <span className="hero__mot" style={{ '--d': '0.16s' }}>véhicule</span>
            </span>
            <span className="hero__titre-ligne" ref={ligne2Ref}>
              <span className="hero__mot hero__mot--accent" style={{ '--d': '0.26s' }}>idéal</span>
              <span className="hero__mot" style={{ '--d': '0.34s' }}>pour</span>
              <span className="hero__mot" style={{ '--d': '0.42s' }}>chaque</span>
              <span className="hero__mot" style={{ '--d': '0.5s' }}>trajet</span>
            </span>
          </h1>
          ...
          <div className="hero__bas">
            <p className="hero__sous-titre">
              Flotte variée · Tarifs clairs · 7j/7<br />
              Au départ de Gex
            </p>
          ...
          <div className="hero__stats">
            {[
              ...
              <div key={i} className="hero__stat" style={{ '--sd': `${0.6 + i * 0.1}s` }}>
      ...
      <section className="categories-accueil">
        <div className="conteneur">
          <h2 className="titre-section reveal">
            Par <span>catégorie</span>
          </h2>
          <p className="sous-titre-section reveal" style={{ transitionDelay: '0.08s' }}>
            Trouvez le véhicule adapté à votre besoin du moment.
          </p>
          <div className="bento">
            {categoriesAvecCompte.map((cat, i) => (
              ...
              >
                ...
                <div className="bento__contenu">
                  <span className="bento__icone">{iconeCategorie[cat.id]}</span>
      ...
      <section className="cta-final">
        <div className="conteneur">
          ...
          <p className="sous-titre-section reveal" style={{ textAlign: 'center', transitionDelay: '0.08s' }}>
            Consultez notre catalogue et choisissez votre véhicule en quelques clics.
          </p>
          <div className="reveal" style={{ textAlign: 'center', transitionDelay: '0.16s' }}>
            <Link to="/catalogue" className="btn btn--primaire btn--grand">
